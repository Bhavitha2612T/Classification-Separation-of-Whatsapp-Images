...
def trainCNN(): 
    ...
    cnn_model.add(MaxPooling2D(pool_size = (2, 2))) 
    cnn_model.add(Flatten())#converting multi dimension array to 2dimension arraay
...
#compiling, training and loading model
    ...
    if os.path.exists("model/model_weights.hdf5") == False: 
        model_check_point =
    ModelCheckpoint(filepath='model/model_weights.hdf5', verbose = 1, save_best_only = True)
    ...
    conf_matrix = confusion_matrix(testY, predict) plt.figure(figsize =(6, 6))


    ax = sns.heatmap(conf_matrix, xticklabels = labels, yticklabels = labels, annot = True, cmap="viridis" ,fmt ="g");
    ax.set_ylim([0,len(labels)])
    plt.title("CNN Whatsapp Image Classification Confusion matrix") plt.ylabel('True class')
